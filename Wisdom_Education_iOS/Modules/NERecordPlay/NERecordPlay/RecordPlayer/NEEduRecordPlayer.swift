...
public class NEEduRecordPlayer: NEEduRecordPlayerProtocol {
    ...
    /// 辅流事件开始时间 - 用户进入课堂时间，单位:秒
    /// 因为辅流录制是从用户进入时间开始的，但播放是收到辅流事件开始的，故需要在开始播放时，seek这个偏移量
    public var subStreamOffSet: Double = 0.0
    ...
    public func play() {
        player.play()
        if subStreamOffSet > 0 {
            player.setCurrentPlaybackTime(subStreamOffSet)
        }
    }
    ...
    @objc public func onPreparedToPlay(noti:Notification) {
        ...
        if self.player.hashValue == player.hashValue {
            print("player通知 onPrepared:\(self.player) 通知：\(noti.object)")
