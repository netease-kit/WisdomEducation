...
abstract class BigClassBaseActivity(layoutId: Int) : BaseClassActivity(layoutId) {
    ...
    fun switchStuLocalHandsUp() {
        eduManager.getMemberService().getLocalUser()?.apply {
            if (isHandsUp()) {
                ConfirmDialog.show(this@BigClassBaseActivity,
                ...
                ConfirmDialog.show(this@BigClassBaseActivity,
                    this@BigClassBaseActivity.getString(R.string.hands_up_apply),
                    this@BigClassBaseActivity.getString(R.string.hands_up_apply_msg),
                    cancelable = true,
                    cancelOnTouchOutside = true,
                    callback = object : ConfirmDialog.Callback {
                        override fun result(boolean: Boolean?) {
