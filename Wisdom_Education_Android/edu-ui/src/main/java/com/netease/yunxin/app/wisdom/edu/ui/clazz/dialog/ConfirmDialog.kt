 ...
 *
 */
open class ConfirmDialog : AlertDialog {
    interface Callback {
    ...
    protected constructor(context: Context, themeResId: Int = R.style.CommonDialogStyle) : super(context,
        themeResId)
    ...
    private fun findViews(inflater: LayoutInflater): View {
        val view: View = inflater.inflate(R.layout.dialog_confirm_layout, null)
    ...
    fun setCancel(cancel: CharSequence?) {
        if (cancelView != null) {
            cancelView!!.text = if (TextUtils.isEmpty(cancel)) context.getString(R.string.cancel) else cancel
        } else {
            this.cancel = cancel
        }
    }
    ...
    fun setOk(ok: CharSequence?) {
        if (okView != null) {
            okView!!.text = if (TextUtils.isEmpty(ok)) context.getString(R.string.sure) else ok
        } else {
            this.ok = ok
        }
    }
    ...
    companion object {
        fun show(
            context: Context, title: String?, message: CharSequence, cancelable: Boolean,
            cancelOnTouchOutside: Boolean, callback: Callback?,
        ): ConfirmDialog {
            return show(context, title, message, null, null, cancelable, cancelOnTouchOutside, callback)
        }
