...
import { RoleTypes, RoomTypes, HandsUpTypes } from '@/config';
...
const VideoPlayer: React.FC<VideoPlayerProps> = observer(({
  ...
  const isBySelf = useMemo(() => userUuid === roomStore?.localData?.userUuid, [roomStore?.localData, userUuid])
    ...
    return (
      <ul onClick={() => setMoreVisible(false)}>
          ...
          <li><Button onClick={() => handleSetAllowScreen(userUuid, 1)} type="text">授予共享权限</Button></li>}
        {
          ...
          <li><Button onClick={() => handsUpAction(userUuid, HandsUpTypes.teacherOff)} type="text">请他下台</Button></li>
        }
      </ul>
    );
  }, [userUuid, roomStore?.roomInfo?.sceneType, wbDrawEnable, canScreenShare, avHandsUp]);
  ...
  const getUserStats = async () => {
    let stats;
    if (isLocal) {
      const result = await roomStore.client?.getLocalVideoStats('screen') || [];
      stats = result[0];
    } else {
      const result = await roomStore.client?.getRemoteVideoStats('screen') || {};
      stats = result[rtcUid];
    }
  ...
  const playVideo = async (
    ...
    if (playDOM?.current?.clientWidth && (((CaptureResolutionWidth || RecvResolutionWidth) > 0 && hasScreen) || hasVideo)) {
      ...
      if (scale >= 1) {
        ...
        height = playDOM.current.clientWidth / scale;
      } else {
        width = playDOM.current.clientHeight * scale;
      ...
      try {
        if (isLocal) {
          logger.log('localplay后设置远端视图', playDOM.current , modeOptions, basicStream)
          // setTimeout(async() => {
          // param.audio = false;
          await basicStream.play(playDOM.current, videoParams).catch((error) => {
            logger.error('play失败1', error)
          });
          await basicStream.setLocalRenderMode(modeOptions.options, modeOptions.mediaType);
          // }, 2000);
        } else {
          // setTimeout(async () => {
          logger.log('play后设置远端视图', playDOM.current , modeOptions, basicStream)
          await basicStream.play(playDOM.current, videoParams).catch((error) => {
            logger.error('play失败2', error)
          });
          await basicStream.setRemoteRenderMode(modeOptions.options, modeOptions.mediaType);
          // }, 2000);
        }
        logger.log('当前播放状态', rtcUid, await basicStream.isPlaying('video'))
      } catch (error) {
        logger.error('play失败4', error)
      }
    } else if (reloadTime < 10) {
      setReloadTime(reloadTime + 1);
      logger.error('play失败-重新渲染',`${reloadTime}次`, playDOM?.current?.clientWidth, CaptureResolutionWidth, RecvResolutionWidth)
      timer = setTimeout(() => {
        playVideo(
          CaptureResolutionWidth,
  ...
  useEffect(() => {
    ...
    if ((basicStream && hasVideo) || (basicStream && hasScreen && (userStats?.CaptureResolutionWidth || userStats?.RecvResolutionWidth))) {
      playVideo(
        userStats?.CaptureResolutionWidth,
        userStats?.CaptureResolutionHeight,
        userStats?.RecvResolutionWidth,
    ...
    basicStream, hasVideo, hasScreen,
    userStats?.CaptureResolutionWidth,
    userStats?.CaptureResolutionHeight,
    userStats?.RecvResolutionWidth,
    userStats?.RecvResolutionHeight
  ...
  return (
    <div className="video-player-component">
      <div className={`video-contaniner video-contaniner-${rtcUid}-${hasScreen? 'screen' : 'video'} ${ hasScreen ? '' : hasVideo ? '' : 'video-hide'}`} ref={playDOM} />
      {!hasVideo && !needDefaultAvatar && <div className="default-video-avatar" />}
      {showMediaStatus &&
      <div className="video-media-control">
        {
          userName ? (<div className="username">{userName}{`${role === RoleTypes.host ? '（老师）' : '（学生）'}`}</div>) : (<div />)
        }
        {
          userName ? (
            <div>
              ...
              {
                hasVideo ? (<i className="media-video-open" />) : (<i className="media-video-close" />)
              }
            </div>
          ) : (<div />)
        }
      ...
      {showUserControl && <div className="video-user-control">
        ...
        <div className="menu-item">
          <Button
            ...
            shape="circle"
            onClick={handleVideo}>
            {hasVideo ? (<i className="contrl-video-open" />) : (<i className="contrl-video-close" />)}
          </Button>
          <p>{hasVideo ? '关闭视频' : '开启视频'}</p>
        </div>
        {showMoreBtn && <div className="menu-item">
          <Popover placement="bottomRight" visible={moreVisible} overlayClassName="video-more-outer" content={<MoreContent />} trigger="click">
            <Button onClick={(e) => {e.stopPropagation();setMoreVisible(true)}} className="control-button" shape="circle">
        ...
        >
          ...
          <p className="desc">结束该学生的上台动作，同时收回他的屏幕共享、白板权限</p>
        </Modal>
      </div>}
    </div>
  )
