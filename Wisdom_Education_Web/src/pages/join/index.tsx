...
const Join:FC = observer(() => {
  ...
  const handleFormChange = (changedValues, allValues) => {
    const formValue = Object.keys(allValues).some(item => allValues[item] === undefined || allValues[item] === '');
    const verifyNum = !roomUuidReg.test(allValues['roomUuid']) && allValues.roomUuid.length === 1;
    if (changedValues.roomUuid !== undefined) {
      if (roomUuidReg.test(changedValues.roomUuid) || changedValues.roomUuid === '') {
      ...
      <div className="wrapper-join">
        <div className="wrapper-joinText">
          ...
          <div className="joinText-download">
            安装包
            <a href="https://pgyer.netease.im/s/RqHj"><img className="imgAndroid" src={android} alt="android"/></a>
            <a href="https://pgyer.netease.im/s/cp7S"><img className="imgIos" src={ios} alt="ios"/></a>
          </div>
        </div>
